...
# 加载训练，验证，测试数据集
# train_ns = pd.read_csv('data2/train_ns.csv')
# val_ns = pd.read_csv('data2/val_ns.csv')
# test_ns = pd.read_csv('data2/test_ns.csv')
train_ns = load_data('data2/train_ns.csv')
val_ns = load_data('data2/val_ns.csv')
test_ns = load_data('data2/test_ns.csv')

train_ns['vec'] = train_ns.text.apply(lambda x: text_to_vec(x, model_w2v))
val_ns['vec'] = val_ns.text.apply(lambda x: text_to_vec(x, model_w2v))
test_ns['vec'] = test_ns.text.apply(lambda x: text_to_vec(x, model_w2v))
